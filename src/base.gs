...
function setDefaultRetailer (retailer) {
  DefaultRetailer = retailer;
}

function setDefaultPurpose (purpose) {
  DefaultPurpose = purpose;
}

function setTitleLogoUrl (titleLogoUrl) {
  if (!titleLogoUrl) {
    return TitleLogoTag = null;
  }
  TitleLogoTag = `<img style="display:block; margin:20px auto;" src="${titleLogoUrl}">`;
}

function setFabiconUrl (fabiconUrl) {
  if (!fabiconUrl) {
    return FabiconUrl = null;
  }
  FabiconUrl = fabiconUrl;
}
...
function execute () {
  ...
  htmlOutput.setFaviconUrl(FabiconUrl);
